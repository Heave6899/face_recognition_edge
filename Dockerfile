...
RUN pip install --no-cache-dir -r ${FUNCTION_DIR}/requirements.txt -f https://download.pytorch.org/whl/cpu/torch_stable.html --target ${FUNCTION_DIR}
RUN chmod 777 /entry.sh
RUN ls ${FUNCTION_DIR}/
RUN apt autoremove
ENV TORCH_HOME /tmp
ENTRYPOINT [ "/entry.sh" ]
CMD [ "eval_face_recognition.face_recognition_handler" ]